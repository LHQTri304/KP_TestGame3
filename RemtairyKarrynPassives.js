...
Game_Actor.prototype.meetsPassiveReq = function(skillId, value) {
	...
	if(ConfigManager.cheatActorNoPassiveGain && Prison.freeMode()) return false;
	if(!value) value = 0;
...
Game_Actor.prototype.relationsPassiveCount = function() {
	...
	if(this.hasPassive(PASSIVE_TRAINER_GYMGOER_SEX_COUNT_TWO_ID)) count += 2;
	else if(this.hasPassive(PASSIVE_TRAINER_GYMGOER_SEX_COUNT_ONE_ID)) count += 1;
	
	if(this.hasPassive(PASSIVE_TRAINER_HORNY_ADVICE_COUNT_ONE_ID)) count += 1;
	
	if(this.hasPassive(PASSIVE_TRAINER_PULL_SHORTS_COUNT_ONE_ID)) count += 1;
...
Game_Actor.prototype.checkHappyEndCondition = function() {
	...
	if(!this.hasPassive(PASSIVE_FIRST_KISS_ID) && !this.hasPassive(PASSIVE_FIRST_SEX_ID) && !this.hasPassive(PASSIVE_FIRST_ANAL_SEX_ID) && mapReactionScore < VAR_DEF_RS_LV2_REQ)
		mapReactionScore -= 10;
...
Game_Party.prototype.addRecordSubdued = function(enemySubdued) {
	if(!enemySubdued || enemySubdued._tagDontCountSubdued || enemySubdued._enemyTempRecordSubduedCounted || enemySubdued._enemyTempRecordSubduedWithEventCommand) return;
...
Game_Actor.prototype.addToActorSwallowMLRecord = function(ml) {
	...
	this._playthroughRecordTotalEjaculationML += ml;
	if(mouthSemen > this._playthroughRecordSwallowMaxML) this._playthroughRecordSwallowMaxML = mouthSemen;
...
Game_Actor.prototype.addToActorPussyCreampieMLRecord = function(ml) {
	...
	this._playthroughRecordTotalEjaculationML += ml;
	if(pussySemen > this._playthroughRecordPussyCreampieMaxML) this._playthroughRecordPussyCreampieMaxML = pussySemen;
...
Game_Actor.prototype.addToActorAnalCreampieMLRecord = function(ml) {
	...
	this._playthroughRecordTotalEjaculationML += ml;
	if(analSemen > this._playthroughRecordAnalCreampieMaxML) this._playthroughRecordAnalCreampieMaxML = analSemen;
...
Game_Actor.prototype.calculateMasochismSensitivityRating = function() {
	...
	if(this.hasPassive(PASSIVE_MASOCHISM_ORGASM_ONE_ID)) rating += this.cockDesire * 0.002;
...
Game_Actor.prototype.karrynPassiveTalkElementRate = function() { //Talk Resist
	...
	if(this.hasPassive(PASSIVE_TRAINER_PULL_SHORTS_COUNT_ONE_ID)) elementBonus += 0.05;
...
Game_Actor.prototype.karrynPassivePettingElementRate = function() { //Petting Resist
	...
	if(this.hasPassive(PASSIVE_TRAINER_GYMGOER_SEX_COUNT_THREE_ID) && this._yesterdayGymTrainerSexDuringWorkCount > 0)
...
Game_Actor.prototype.kissingMouthDesireRequirement = function(kissingLvl, karrynSkillUse) { 
	...
	if(this.isEquippingThisAccessory(RING_FAIRY)) req -= 15;
...
Game_Actor.prototype.handjobCockDesireRequirement = function(karrynSkillUse) { 
	...
	if(this.isEquippingThisAccessory(RING_FAIRY)) req -= 10;
...
Game_Actor.prototype.calculateMaxButtDesire = function() { //max butt desire
	...
	else if(this.hasPassive(PASSIVE_ANAL_PETTED_COUNT_ONE_ID)) upperLimit += 5;
...
Game_Actor.prototype.calculateMaxCockDesire = function() { //max cock desire
	...
	if(this.hasPassive(PASSIVE_DEFEATED_COUNT_TWO_ID)) upperLimit += 10;
	...
	if(this.hasPassive(PASSIVE_TITTYFUCK_COUNT_TWO_ID)) upperLimit += 10;
...
Game_Actor.prototype.startingMouthDesire = function() { //Starting Mouth
	...
	if(this.hasPassive(PASSIVE_PETTING_ORGASM_TWO_ID)) desire += 7;
	else if(this.hasPassive(PASSIVE_PETTING_ORGASM_ONE_ID)) desire += 3;
...
Game_Actor.prototype.jobDesireMultipler = function() {
	...
	if(this.isInJobPose() && !$gameParty.isInGloryBattle) {
		...
		if($gameParty.isInWaitressBattle) {
			...
			if(this.hasPassive(PASSIVE_TRAINER_HORNY_ADVICE_COUNT_ONE_ID)) specificJobMulti += 0.15;
			
			if(this.hasPassive(PASSIVE_TRAINER_GYMGOER_SEX_COUNT_TWO_ID)) {
				let todayCockTotal = this._todayCockKickUsageCount + this._todayCockStareUsageCount + this._todayCockPetUsageCount;
				if(todayCockTotal) {
	...
	case ELEMENT_STRIP_ID:
		if(this.hasPassive(PASSIVE_MAX_PUSSY_DESIRE_FIRST_ID) && this.pussyDesire >= 50 && !this.isInMapPose()) {
			...
			if(this.pussyDesire >= 100) {
				...
				rate += 0.05 - charmEffect + this.pussyDesire * 0.001;
			}
		}
	break;
...
Game_Actor.prototype.passiveRegenEffects = function() {  
	...
	if($gameParty.isInTrainerBattle) {
		if(this.isInTrainerRinkanPose()) {}
		else hornyChance *= 1.25;
	}
	else if(this.isInJobPose()) hornyChance *= 0.75;
	else if(this.isInMasturbationCouchPose()) hornyChance *= 0.4;
...
Game_Enemy.prototype.enemyPassiveXParamRate = function(id) {  
	...
	if(this.isStateAffected(STATE_ENEMY_KISSED_ID) && (id === XPARAM_HIT_ID || id === XPARAM_EVA_ID)) {
		...
		if(Karryn.isUsingThisTitle(TITLE_ID_SEX_SKILL_KISS_TWO)) kissDebuffLvl += 1;
