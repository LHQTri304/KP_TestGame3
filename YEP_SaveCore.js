  ...
  for (var i = 0; i < length; ++i) {
    ...
    if (member) {
      if (Yanfly.Param.SaveInfoPartyType === 1) {
        ...
        var name = member.saveFileFaceName();
        var index = member.saveFileFaceIndex(this._saveContents.switches, this._saveContents.map._mapId);
...
Window_SaveInfo.prototype.drawPartyLevels = function(dy) {
  ...
  */
	...
	if(this._saveContents.actors._data[ACTOR_KARRYN_ID]) {
		...
		else if(TextManager.isRussian) lvText += ' ';
		lvText += '' + wardenLv;
...
Window_SaveInfo.prototype.drawColumn = function(column, dx, dy, dw) {
    ...
    for (let i = 0; i < length; ++i) {
      ...
      if (data.toUpperCase().trim() !== 'EMPTY') {
		...
		(data.toUpperCase().trim() === 'TOTAL GAMECLEARS' && !this._saveContents.party._totalGameClears)) {
			dy += this.lineHeight();
			continue;
		}
...
Window_SaveInfo.prototype.drawData = function(data, dx, dy, dw) {
  if (data.toUpperCase().trim() === 'NULL') {
	...
	} else if (data.toUpperCase().trim() === 'TOTAL GAMECLEARS') {
...
Window_SaveInfo.prototype.drawLocation = function(dx, dy, dw) {
    ...
    if (Yanfly.Param.SaveMapDisplayName) {
		...
		*/
		text = DataManager.getMapLocationDisplayNameRem(id);
...
Window_SaveInfo.prototype.drawDifficulty = function(dx, dy, dw) {
	...
	let freePlay = this._saveContents.switches._data[SWITCH_FREE_MODE_ID] === true;
	let challengeRun = this._saveContents.switches._data[SWITCH_TITLE_LOCK] === true;
	let descensionRun = false;
	
	if(this._saveContents.party.normalMode()) {
		this.changeTextColor(this.textColor(2));
		if(freePlay)
			difficulty = TextManager.WardenFreePlayMode;
		else if(challengeRun)
			difficulty = TextManager.WardenChallengeRun;
		else if(descensionRun)
			difficulty = TextManager.WardenDescensionRun;
		else
		...
		if(freePlay)
			difficulty = TextManager.PrisonerFreePlayMode;
		else if(challengeRun)
			difficulty = TextManager.PrisonerChallengeRun;
		else if(descensionRun)
			difficulty = TextManager.PrisonerDescensionRun;
		else
			difficulty = TextManager.PrisonerMode;
	}
	else {
		if(freePlay)
			difficulty = TextManager.SecretaryFreePlayMode;
		else if(challengeRun)
			difficulty = TextManager.SecretaryChallengeRun;
		else if(descensionRun)
			difficulty = TextManager.SecretaryDescensionRun;
		else
			difficulty = TextManager.SecretaryMode;
	}
...
Window_SaveInfo.prototype.drawTotalGameClears = function(dx, dy, dw) {
	if(!this._saveContents.party._totalGameClears) return;
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.yanflySaveTotalGameClears, dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());  
    this.drawText(this._saveContents.party._totalGameClears, dx, dy, dw, 'right');
};
